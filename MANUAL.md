...
- **Memory** — explorar lo guardado. A la izquierda de la fila de arriba, **cómo se ve**: un solo menú con *Tarjetas* (la lista), *Temas*, *Lugares*, *Tiempo*, *Relaciones* (el grafo) y *Media*. Contra el borde derecho, separado por una línea, **con qué se acota**: proyecto, tema, tag, tipo de memoria y fecha. Todos arrancan en *Todos* —sin filtrar— y el botón dice en qué está: al cambiarlo muestra el valor elegido y se enciende. Los filtros están siempre a la vista y acotan todas las vistas —tarjetas, temas, lugares, tiempo, relaciones y media muestran solo lo que cumple los cinco a la vez—, no solo la búsqueda; los menús de tema y tag se arman con lo que las memorias tienen de verdad y traen su propio buscador adentro.
- Tres vistas tienen además su propia opción:
  - **Lugares** elige qué lugar mira: el *del contenido* (de qué habla la memoria) o el *de captura* (dónde estabas al anotarla). Lo que no tenga el que elegiste queda en **Sin lugar**, un cajón más de la lista que se abre como cualquier lugar.
  - **Tiempo** elige qué fecha mira: la *del contenido* (de cuándo habla la memoria) o la *de grabación* (cuándo entró a MeM). La línea temporal se estira entre la primera y la última fecha de lo que estás mirando, y lo que no tenga esa fecha sale abajo, en **Sin fecha**.
  - **Relaciones** pinta cada tipo de conexión de su color —tema, tag, lugar, enlace y similitud—, el mismo color del botón que la prende y apaga en ⚙ *Filtros*. Pasando el mouse por una línea dice qué tipo de relación es y cuál: «temas · Creaciones/Imagen — esta memoria ↔ Imagen».
