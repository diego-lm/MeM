...
- **Dónde manda cada rama**: `buscar` (MCP, chat, `ask`) devuelve lo semántico —esa es la gracia—. El **omnibox de la app** (`/memory/search?orden=relevancia`) es un filtro: solo salen las memorias que DICEN lo que se escribió, y el híbrido únicamente las ordena. Es a propósito: sin umbral de coseno el híbrido devuelve siempre sus vecinos más cercanos —medido en la base real, hasta `xyzzy` saca cinco— y mezclados con los literales la lista se leía como si escribir no filtrara nada (2026-09-06). Con 1-2 letras el filtro es por principio de palabra sobre título/tags/subjects/lugar; de 3 en adelante, substring sobre todo el texto.
...
Todas derivan de la MISMA lista filtrada (`visibles` en `screens/memory.js`): los cinco chips de arriba —proyecto, tema, tag, tipo y ventana de fecha— se aplican una sola vez y de ahí cuelgan tarjetas, temas (sus contadores se cuentan en el cliente, no los manda `/memory/tree`), lugares, tiempo, relaciones y media; el grafo y los mapas reciben ese recorte como `ocultas`. Además de Recientes/Temas/Proyectos: **Tiempo** (lista por bloques o línea temporal con zoom, con un selector de QUÉ fecha se mira: la del contenido (`cuando`) o la de grabación (`capturado ?? mtime`); el eje se estira entre la primera y la última de lo que se está mirando y lo que no tiene esa fecha va abajo, en "Sin fecha" — la capa del instante de captura sigue como opción de la línea), **Lugares** (selector de QUÉ lugar se mira —`lugar`, de qué habla la memoria, o `lugar_captura`/`coords_captura`, desde dónde se anotó— y para el elegido chips o mapa Leaflet/OSM: círculos ● para el del contenido, rombos ◇ para el de captura; lo que no lo tiene cae en el cajón "Sin lugar", que se abre como cualquier otro), **Grafo** (la base entera como red d3-force: las memorias se conectan A TRAVÉS de nodos-eje que dicen por qué —subject ●, tag ○, lugar ◇, un eje por valor compartido por dos o más—, más `[[wikilink]]` ⇄ y similitud semántica ┈; cada tipo de relación tiene su color —el mismo en la arista, en el nodo-eje y en el chip que lo prende— y el hover de una arista dice qué tipo es y cuál (`temas · Creaciones/Imagen — A ↔ B`); un eje que se queda sin memorias visibles se cae con ellas, y dos deslizadores acotan el dibujo: **parentesco** (cuántos saltos desde el centro) y **cercanía** (el coseno mínimo de las aristas semánticas). Tap en memoria abre la ficha, tap en un eje resalta su vecindario; el zoom con rueda o dos dedos crece desde el punto que se toca), **Mapa semántico** (proyección 2D de los embeddings: lo parecido queda cerca) y **◫ Canvas** (tableros de brainstorming: tarjetas arrastrables de memorias y notas; "✦ Sugerir" mezcla el tablero con vecinas semánticas y memorias lejanas al azar y le pide ideas al LLM, cada idea citando sus fuentes). Todo 2D, todo vendorizado (d3-force, Leaflet), sin CDN.
